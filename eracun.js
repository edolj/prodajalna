...
var strankaId = function(izbraniCustomer,callback) {
    pb.all("SELECT Customer.* FROM Customer, Invoice WHERE Customer.CustomerId = " + izbraniCustomer,
    function(napaka, vrstice) {
        callback(vrstice); 
    })
}
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  strankaId(zahteva.session.prijavljen, function(narocnik) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    if (!pesmi) {
      odgovor.sendStatus(500);
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    var napaka2 = false;
    sporociloSpr = "Stranka je bila uspešno registrirana.";
    ...
    try {
      ...
      napaka2 = true;
      sporociloSpr = "Prišlo je dp napake pri registraciji nove stranke. \
...
streznik.get('/prijava', function(zahteva, odgovor) {
  vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        odgovor.render('prijava', {sporocilo: sporociloSpr, seznamStrank: stranke, seznamRacunov: racuni});
        sporociloSpr = "";
      }) 
  });
})
